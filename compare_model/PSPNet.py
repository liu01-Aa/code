...
from net.pspnet.mobilenetv2 import mobilenetv2
from net.pspnet.resnet import resnet50
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor,out_classes=12, backbone="resnet50", pretrained=True, aux_branch=True):
        super(PSPNet, self).__init__()
        norm_layer = nn.BatchNorm2d
        self.final = feHead(in_chs = num_classes, out_chs=out_classes)
    ...
    def forward(self, x):
        ...
        seg_head, ins_head, count = self.final(output)
        return seg_head, ins_head, count
        # if self.aux_branch:
...
# net = cm.PSPNet(num_classes=20, downsample_factor=16).cuda()                   