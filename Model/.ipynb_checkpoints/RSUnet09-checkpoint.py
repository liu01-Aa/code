...
class RC_skip(nn.Module):
    def __init__(self, in_chans=256, out_chans=128,  conv_op=nn.Conv2d):
        ...
        self.up_d1_2 = single_conv_relu_batch(out_chans+out_chans//2, out_chans//2,  kernel_size=3, padding=1, conv_op=conv_op)
        self.up_d2_1 = single_conv_relu_batch(in_chans, out_chans,  kernel_size=3, padding=1, conv_op=conv_op)
        self.up_d2_2= single_conv_relu_batch(in_chans+out_chans, out_chans, kernel_size=3, padding=1, conv_op=conv_op)
        self.up_d3 = single_conv_relu_batch(in_chans*2, in_chans, kernel_size=3, padding=1, conv_op=conv_op)
        
        self.conv1x1 = single_conv_relu_batch(out_chans+out_chans//2, out_chans//2, kernel_size=1, stride=1, padding=0, conv_op=conv_op)
