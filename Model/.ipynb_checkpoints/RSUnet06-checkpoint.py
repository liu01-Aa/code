...
class Decoder_UP0(nn.Module):
    ...
    def forward(self, x_4_0):
        y = F.interpolate(x_4_0, scale_factor=2, mode='bilinear', align_corners=True)
        x_3_1 = self.up0(y) # 256
        return x_3_1
...
class Decoder_UP1(nn.Module):
    ...
    def forward(self, x_3_1, x_4_0):
        x_3_1 = F.interpolate(x_3_1, scale_factor=2, mode='bilinear', align_corners=True)   #对f3进行上采样，将其尺寸增加一倍，使用双线性插值法，并确保角点对齐

        x_4_0 = self.conv1x1(x_4_0)

        x_4_0 = F.interpolate(x_4_0, scale_factor=4, mode='bilinear', align_corners=True)  #对 f4进行上采样，将其尺寸增加四倍，使用双线性插值法，并确保角点对齐

        x = torch.cat([x_3_1, x_4_0], dim=1) # 128 x 3
        x_2_2 = self.up(x)      #将拼接后的特征 x传递给双卷积层进行特征提取和变换，得到最终的特征
        return x_2_2
...
class Decoder_UP2(nn.Module):
    ...
    def forward(self, x_2_2, x_3_1):
        x_2_2 = F.interpolate(x_2_2, scale_factor=2, mode='bilinear', align_corners=True)

        x_3_1 = self.conv1x1(x_3_1)
        x_3_1 = F.interpolate(x_3_1, scale_factor=4, mode='bilinear', align_corners=True)
        x = torch.cat([x_2_2, x_3_1], dim=1) # 64 64 32 16
        x_1_3 = self.up(x)
        return x_1_3
...
class Decoder_UP3(nn.Module):
    ...
    def forward(self, x_1_3, x_2_2):
        x_1_3 = F.interpolate(x_1_3, scale_factor=2, mode='bilinear', align_corners=True)

        x_2_2 = self.conv1x1(x_2_2)
        x_2_2= F.interpolate(x_2_2, scale_factor=4, mode='bilinear', align_corners=True)
        x = torch.cat([x_1_3, x_2_2], dim=1) # 32  32 16 8 4
        x_0_4 = self.up(x)
        return x_0_4

class RsUnet06(nn.Module):
    def __init__(self, in_chans=[64, 128, 256, 512, 1024], out_chs=20,  out_class=2, deep_supervision=False):
        super(RsUnet06, self).__init__()
        ...
        self.upsample_3_1 = Decoder_UP0(in_chans=1024, out_chans=512) 
 
        self.upsample_2_1 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=4, stride=2, padding=1) 
        self.upsample_2_2 = Decoder_UP1(in_chans=512, out_chans=256) 
        ...
        self.upsample_1_3 = Decoder_UP2(in_chans=256, out_chans=128) 
        ...
        self.upsample_0_4 = Decoder_UP3(in_chans=128, out_chans=64) 
...
#         self.final_super_0_4 = feHead(in_chs=64, out_chs=num_classes)
    ...
    def forward(self, x):
        ...
        x_2_2 = torch.cat([self.upsample_2_2(x_3_1,x_4_0), x_2_0, x_2_1], 1)
        x_2_2 = self.CONV2_2(x_2_2)
        
        x_1_2 = torch.cat([self.upsample_1_2(x_2_1), x_1_0, x_1_1], 1)
        x_1_2 = self.CONV1_2(x_1_2)
        
        x_1_3 = torch.cat([self.upsample_1_3(x_2_2,x_3_1), x_1_0, x_1_1, x_1_2], 1)
        ...
        x_0_4 = torch.cat([self.upsample_0_4(x_1_3,x_2_2), x_0_0, x_0_1, x_0_2, x_0_3], 1)
