...
class cross_conv(nn.Module):
    def __init__(self, in_ch, out_ch, conv_op=nn.Conv2d):
        ...
        self.conv1 = singleConv(in_ch, out_ch, kernel_size=3, stride=1, padding=1, conv_op=nn.Conv2d)  # 降维
...
class RC_skip(nn.Module):
    def __init__(self, in_chans=256, out_chans=128,  conv_op=nn.Conv2d):
        ...
        self.convtrans_d3 = nn.ConvTranspose2d(in_chans*2, out_chans*2, kernel_size=4, stride=2, padding=1)
        self.convtrans_d2_1 = nn.ConvTranspose2d(in_chans, out_chans, kernel_size=4, stride=2, padding=1)
        self.convtrans_d2_2 = nn.ConvTranspose2d(in_chans, out_chans, kernel_size=4, stride=2, padding=1)

        self.up_d2_1 =  double_conv_relu_batch(in_chans, out_chans,  kernel_size=3, padding=1, conv_op=conv_op)
        self.up_d2_2=  double_conv_relu_batch(in_chans, out_chans, kernel_size=3, padding=1, conv_op=conv_op)
        self.up_d3 =  double_conv_relu_batch(in_chans*2, in_chans, kernel_size=3, padding=1, conv_op=conv_op)
        # self.conv1x1 = single_conv_relu_batch(in_chans+out_chans, out_chans, kernel_size=1, stride=1, padding=0, conv_op=conv_op)
    def forward(self, d2, d3, d4):  # d2, d3, d4 网络深度节点输出
        d4 = self.convtrans_d3(d4)
        x = torch.cat([d3, d4], dim=1)  # 512
        x = self.up_d3(x)

        d3 = self.convtrans_d2_1(d3)
        x2_1 = torch.cat([d2, d3], dim=1)  # 512
        x2_1 = self.up_d2_1(x2_1)

        x = self.convtrans_d2_2(x)
        x2_2 = torch.cat([d2, x], dim=1)  # 512
        x2_2 = self.up_d2_2(x2_2)

        x = torch.cat([d2, x2_1, x2_2], dim=1)
        # x = self.conv1x1(x)
...
class Decoder_UP1(nn.Module):
    def __init__(self, in_chans=256, out_chans=128, conv_op=nn.Conv2d):
        ...
        # self.conv1x1 = single_conv_relu_batch(in_chans*3, out_chans, kernel_size=1, stride=1, padding=0, conv_op=conv_op)
...
class Decoder_UP2(nn.Module):
    def __init__(self, in_chans=128, out_chans=64, conv_op=nn.Conv2d):
        ...
        self.up = double_conv_relu_batch(in_chans+out_chans*3, out_chans, kernel_size=3, padding=1,  conv_op=conv_op)
...
class Decoder_UP3(nn.Module):
    ...
    def forward(self, f1, f2):
        ...
        x = torch.cat([f1, f2], dim=1) # 32  32 16 8 4
...
class InstanceCounter(nn.Module):
    def __init__(self, input_n_filters, out_chs=16, usegpu=True):
        ...
        # self.cnn = nn.Sequential(
        #                          nn.Conv2d(input_n_filters, self.out_filter, 1, 1, 0),
        #                          nn.AdaptiveAvgPool2d((1, 1))
        #                          )
...
class RsUnet(nn.Module):
    ...
    def forward(self, x):
        ...
        inter = []
        inter.append(features[0])

        s = self.rc_skip(features[2], features[1], features[0])
        inter.append(s)

        f4 = x
        f3 = self.up0(inter[0], x)
        f2 = self.up1(f3, f4)
        f1 = self.up2(inter[1], f2)
        f0 = self.up3(f1, f2)
