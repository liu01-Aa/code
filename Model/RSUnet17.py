...
class Resnet18(nn.Module):
    ...
    def forward(self, x):
        feature = []
        x = self.stage_0(x) #32 * 256 * 256   [32, 64, 160, 352]
        feature.append(x)

        x = self.stage_1(self.pool(x))
        feature.append(x)

        x = self.stage_2(self.pool(x))
        feature.append(x)

        x = self.stage_3(self.pool(x))
        feature.append(x)

        x = self.stage_4(self.pool(x))
