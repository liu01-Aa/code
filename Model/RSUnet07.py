...
class RsUnet07(nn.Module):
    ...
    def forward(self, x):
        ...
        f4 = x   #将剩余的部分x赋值给 f4
        f3 = self.upsample_3_1(f4)
        f2 = self.upsample_2_2(f3)
        f1 = self.upsample_1_3(f2)
        f0 = self.upsample_0_4(f1)
        f0 = torch.cat([inter[1],f0], 1)
        f0 = self.transition_conv(f0)
