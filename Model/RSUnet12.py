...
class Decoder_UP2(nn.Module):
    ...
    def forward(self, x, f2, f3):
        ...
        x = torch.cat([x, y, f3], dim=1) # 64 64 32 16
        f1 = self.up(x)
        return f1
...
class RsUnet12(nn.Module):
    ...
    def forward(self, x):
        ...
        inter.extend(features)    #将下采样得到的第一个特征添加到 inter列表中，通常是编码器中提取的最底层的特征
        ...
        f1 = self.up2(inter[2],f2, f3)
        f0 = self.up3(inter[3],f1, f2)
