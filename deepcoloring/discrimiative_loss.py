...
# 从输入的标签数据中生成实例掩码（instance_mask）、语义掩码（sematic_mask）以及实例数量列表（n_object_list）
# class Discriminative_binary_Mask(nn.Module): #注释了整个Discriminative_binary_Mask类
#     def __init__(self, max_n_objects=25, sem_class=2):
...
# 将输入的标签数据转换为适合后续处理的实例掩码、二进制语义掩码和实例数量列表
class Discriminative_binary_Mask(nn.Module): #新增整个Discriminative_binary_Mask类--运行A4报错   instance_mask = np.array(instance_mask, dtype='int')
# ValueError: setting an array element with a sequence. The requested array has an inhomogeneous shape after 1 dimensions. The detected shape was (4,) + inhomogeneous part.
    ...
    def _single_discriminative_mask(self, label):
        ...
        label_height, label_width = label.shape
        instance_values = sorted(set(np.unique(label)).difference([0]))
        n_instances = len(instance_values)

        # 直接创建固定大小的数组
        instance_mask = np.zeros((self.max_n_objects, label_height, label_width), dtype=np.uint8)

        # 填充真实实例
        for l, v in enumerate(instance_values[:self.max_n_objects]):  # 确保不超过max_n_objects
            instance_mask[l] = (label == v).astype(np.uint8)

        # 生成语义掩码
        foreground = np.sum(instance_mask, axis=0) > 0
        sem_mask = np.stack([~foreground, foreground]).astype(np.uint8)

        return instance_mask, sem_mask, min(n_instances, self.max_n_objects)
    ...
    def forward(self, labels):
        ...
        for i in range(batch):
            _instance_mask, _sem_mask, _n_instance = self._single_discriminative_mask(labels[i])
            instance_mask_list.append(_instance_mask)
            sem_mask_list.append(_sem_mask)
            n_object_list.append(_n_instance)

        # 转换为张量（现在所有元素形状一致）
        instance_mask = torch.from_numpy(np.array(instance_mask_list)).long().to(device)
        sem_mask = torch.from_numpy(np.array(sem_mask_list)).long().to(device)
        n_object_list = torch.LongTensor(n_object_list).to(device)
